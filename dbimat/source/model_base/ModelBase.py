...
from dbimat.source.basic.Streamtypes import StreamMass, StreamEnergy, StreamDirection
from dbimat.source.basic.Settings import *
from dbimat.source.basic.Quantities import PhysicalQuantity, get_unit_of_quantity
from dbimat.source.basic.Units import Unit
from dbimat.source.basic.CustomErrors import *
# Helper function Imports

from dbimat.source.helper.initialize_logger import initialize_logger, LoggingLevels

# component imports
from dbimat.source.modules import *
from dbimat.source.modules import GenericUnit, Converter, Consumer, Grid
from dbimat.source.modules import Storage_Electrical, Storage, Storage_Gas, Source

# import model includes
import dbimat.source.basic.Streamtypes
from dbimat.source.model_base.Branches import Branch
from dbimat.source.model_base.Dataclasses.EconomicalDataclasses import *
from dbimat.source.model_base.Dataclasses.TechnicalDataclasses import *
from dbimat.source.model_base.Dataclasses.ExportDataclasses import SystemResults, PortResult, ComponentTechnicalResults, ComponentEconResults
import dbimat.source.model_base.Connections2Branches as Connections2Branches
import dbimat.source.model_base.database_connection as database_connection
...
class ModelBase(database_connection.Mixin, Connections2Branches.Mixin):
    ...
    def add_branch(self, branch_name: str, branch_type: StreamEnergy, port_connections: list):
        ...
        self.branches[branch_name] = Branch(branch_type=branch_type, port_connections=port_connections,
                                            basic_technical_settings=self.basic_technical_settings,
    ...
    def solve(self, runcount) -> int:
        ...
        if self.basic_technical_settings.time_resolution is None:
            ...
            """
            while not all([branch.calculated is True for branch in self.branches.values()]) and (
                    timeout < self.basic_technical_settings.timeout_max):
                timeout += 1
        ...
        if self.self_energy_components:
            ...
            for comp in self.self_energy_components['Excluded Consumers']:
                ...
                if len(consumer_ports) > 1:
                    logging.critical(
                        f'Self energy component {comp} has more than one port. Could not define specific port')
                else:
