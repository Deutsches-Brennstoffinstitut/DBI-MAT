...
from dbimat.source.basic.Quantities import PhysicalQuantity, EconomicalQuantities, get_unit_of_quantity
...
class Mixin:
    ...
    def _local_connection(database_name: str):
        path_str=f'dbimat/source/data/{database_name}.sqlite'
        ...
        while True:
            ...
            if path == path_old:
                raise ConnectionError(f'No Connection to local Database "{database_name}" possible. Check if '
                                      f'name or location (dbimat\\source\\data\\{database_name}.sqlite) is correct.')
