...
class Grid(GenericUnit):
    ...
    def run(self, port_id, branch_information, runcount=0):
        ...
        if self.active & (not loop_control):
            if not isinstance(controlled_port.get_profile_values(runcount),
                              dict) and controlled_port.get_profile_values(runcount) is None:
                if controlled_port.get_stream_limits() is None:
                    controlled_port.set_stream(runcount, 0)
                else:
                    controlled_port.set_stream(runcount, controlled_port.get_stream_limits()[1])
            else:
