...
class Storage(GenericUnit):
    ...
    def set_initial_value(self, buffer):
        """
        Set initial filling of Storage
    ...
    def get_load(self):
        ...
        :return: string
        """
        return f"current flow {self.ports[self.controlled_medium]} kW mit current filling status {self.buffer_new} kWh, status: {self.status}"
