...
from dbimat.source.helper.Conversion import Unit_Conversion as uc, basic_calculations as bc, \
...
class Pipeline(GenericUnit):
    ...
    def _blank_fluid(self, fluid_string=None):
        """Initializes the Fluid with its "abstractState"-Class. This method is needed for a blank
        initialization due to the fact that the fluid properties get initialized somewhere else in a simulation of an
        energy system.

        Args:
            fluid_string(str): fluid as string

        """

        if fluid_string is not None:
            self.fluid = AbstractState("HEOS", fluid_string)
...
if __name__ == '__main__':
    ...
    ppln.save_path = "Pipeline_output.xlsx"
